...
public class LoginActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final ProgressDialog pd = ProgressDialog.show(LoginActivity.this, "Please Wait", "Checking for saved login...", false, true);
	...
	private void checkSavedLogin(final ProgressDialog pd) {
		//Temporary until we can find a better way to store the user state -- Maybe store JSON of user object in future
		if (!savedLogin.getString("email", "").contentEquals("") && !savedLogin.getString("password", "").contentEquals("")) {
			...
			Session.getSession().login(email, password, getRegistrationId(this), new Callback<User>() {
				//Login through backend. Hopefully avoid this in the future
				@Override
				...
				public void success(User arg0, Response arg1) {
					//If successful, create new intent, set flags, and start the activity
	...
	private void registerInBackground() {
		new AsyncTask<Void,Void,String>() {
			...
			protected String doInBackground(Void... params) {
				...
				String msg = "";
				try{
					//If no Gcm instance, create a new instance for this context
					if (gcm == null)
					...
					//Not needed currently.  We are sending up the GCM API key upon login
	...
	private boolean checkPlayServices(final ProgressDialog pd) {
	    ...
	    //If not successful attempt to recover from the error and have them download google play services
	 ...
	 * @param v View object representing the view object that called this function
	 */
